        ...
        do
            ...
            do
                ...
                for experiment_number in $(seq $number_of_experiments); do
                    ...
                    command=(        'taskset -c 1 ip netns exec mr_client /root/evaluation/iperf/src/iperf3 -c 10.2.2.101 --bytes "${payload_size}" --parallel "${clients_requests}" --json ${additional_parameters_for_drawing}' )
                                        
                    printf "   1.1.1 Running command: '${command[@]}' \n"
                    command_output=$( taskset -c 1 ip netns exec mr_client /root/evaluation/iperf/src/iperf3 -c 10.2.2.101 --bytes "${payload_size}" --parallel "${clients_requests}" --json  ${additional_parameters_for_drawing} > "${individual_experiment_folder}/result.json" )
