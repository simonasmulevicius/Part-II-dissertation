            ...
            do
                ...
                for experiment_number in $(seq $number_of_experiments); do
                    ...
                    then
                        ...
                        command=(        'taskset -c 1 ip netns exec mr_client iperf3 -c 10.2.2.101 --bytes "${payload_size}" --parallel "${clients_requests}" --json ${additional_parameters_for_drawing}' )
                                            
                        printf "   1.1.1 Running command: '${command[@]}' \n"
                        command_output=$( taskset -c 1 ip netns exec mr_client iperf3 -c 10.2.2.101 --bytes "${payload_size}" --parallel "${clients_requests}" --json  ${additional_parameters_for_drawing} > "${individual_experiment_folder}/result.json" )
                    else
                        printf "  1.1 Run throughput test\n"
                        command=(        'taskset -c 1 ip netns exec mr_client iperf3 -c 10.2.2.101 --bytes "${payload_size}" --parallel "${clients_requests}" ${additional_parameters_for_drawing}' )
                                            
                        printf "   1.1.1 Running command: '${command[@]}' \n"
                        command_output=$( taskset -c 1 ip netns exec mr_client iperf3 -c 10.2.2.101 --bytes "${payload_size}" --parallel "${clients_requests}" ${additional_parameters_for_drawing} > "${individual_experiment_folder}/result.txt" )
                        ...
                        then
                            ...
                            individual_throughput_samples=$( grep " sec " "${individual_experiment_folder}/result.txt"  | sed '$d' | sed '$d' | tr - " " | awk '{
                                if ($9 =="Gbits/sec")
                                    ...
                                    {print $3, $8*/1000/1000}
                                }' | gnuplot -p -e 'set title "TCP throughput (1 client) \n when transfering 1 000 000 000 Bytes"  font "Times-Roman,12" ;   set xlabel "Time (s)"  font "Times-Roman,12"; set ylabel "Throughput over the 1s interval (MBits/s)" font "Times-Roman,12";  set yrange[0:12000] ;  set key font "Times-Roman,12" ; set xtics font "Times-Roman,12" ; plot  "/dev/stdin" title "iperf3 TCP throughput"  with linespoints'       )
