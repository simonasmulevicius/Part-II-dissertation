...
for experiment_number in $(seq $number_of_experiments); do
    ...
    echo "perf record -F ${sampling_frequency} --cpu=1 --call-graph dwarf taskset -c 1 ip netns exec mr_client  ${nghttp2_folder}/src/h2load -P --npn-list h3 https://10.2.2.101:7777/${payload_size}"
          perf record -F ${sampling_frequency} --cpu=1 --call-graph dwarf taskset -c 1 ip netns exec mr_client  ${nghttp2_folder}/src/h2load -P --npn-list h3 https://10.2.2.101:7777/${payload_size}
