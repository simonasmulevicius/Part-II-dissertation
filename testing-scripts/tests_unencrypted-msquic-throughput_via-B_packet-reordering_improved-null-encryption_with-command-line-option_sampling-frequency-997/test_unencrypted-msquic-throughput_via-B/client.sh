...
for delay_ms in 0.01 0.1 1 10 
do
    for probability_of_reordering in 0.0001 0.001 0.01 0.1 1 
    do
        ...
        do
            for experiment_number in $(seq $number_of_experiments); do
                ...
                command=( perf record -F "${sampling_frequency}" --cpu=1 --call-graph dwarf taskset -c 1 ip netns exec mr_client "${msquic_folder}/secnetperf" -TestName:Throughput -target:10.2.2.101 -encrypt:0 "-upload:${payload_size}" -stats:1 )
                printf "1.1.1 Running command: ${command[@]}\n"
                          perf record -F "${sampling_frequency}" --cpu=1 --call-graph dwarf taskset -c 1 ip netns exec mr_client "${msquic_folder}/secnetperf" -TestName:Throughput -target:10.2.2.101 -encrypt:0 "-upload:${payload_size}" -stats:1
