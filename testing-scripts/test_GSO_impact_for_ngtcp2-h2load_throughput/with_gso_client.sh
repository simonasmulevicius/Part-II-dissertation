...
# 1. Remove "-DNGTCP2_ENABLE_UDP_GSO=0" flag from ngtcp2 ./configure script
    ...
    do
        ...
        do
            ...
            do
                ...
                experiment_group_folder="./with_gso_results/${group_experiment_description}"
                ...
                for experiment_number in $(seq $number_of_experiments); do
                    ...
                    command=(        'taskset -c 1 ip netns exec mr_client  "${nghttp2_folder}/src/h2load" --requests "${clients_requests}" --clients "${clients_requests}"    --npn-list h3 "https://10.2.2.101:7777/${payload_size}"' )
                   
                    printf "   1.1.1 Running command: '${command[@]}' \n"
                    command_output=$( taskset -c 1 ip netns exec mr_client  "${nghttp2_folder}/src/h2load" --requests "${clients_requests}" --clients "${clients_requests}"    --npn-list h3 "https://10.2.2.101:7777/${payload_size}"  | tee "${individual_experiment_folder}/result.txt" )
