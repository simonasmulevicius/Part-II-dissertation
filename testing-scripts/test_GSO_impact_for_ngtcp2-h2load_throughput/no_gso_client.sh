...
# 1. ADD "-DNGTCP2_ENABLE_UDP_GSO=0" flag from ngtcp2 ./configure script
    ...
    do
        ...
        do
            ...
            do
                ...
                experiment_group_folder="./no_gso_results/${group_experiment_description}"
                ...
                for experiment_number in $(seq $number_of_experiments); do
                    ...
                    command=(        'taskset -c 1 ip netns exec mr_client  "${nghttp2_folder}/src/h2load" --no-udp-gso --requests "${clients_requests}" --clients "${clients_requests}"    --npn-list h3 "https://10.2.2.101:7777/${payload_size}"' )
                   
                    printf "   1.1.1 Running command: '${command[@]}' \n"
                    command_output=$( taskset -c 1 ip netns exec mr_client  "${nghttp2_folder}/src/h2load" --no-udp-gso --requests "${clients_requests}" --clients "${clients_requests}"    --npn-list h3 "https://10.2.2.101:7777/${payload_size}"  | tee "${individual_experiment_folder}/result.txt" )
                    ...
                    then
                        throughput_megabits_per_second=$(bc -l <<<"${throughput_string::(-4)}*1024*1024*1024/1000/1000*8")
                    
                    elif [ "${throughput_string:(-4)}" == "MB/s" ] 
                    then
                        throughput_megabits_per_second=$(bc -l <<<"${throughput_string::(-4)}*1024*1024/1000/1000*8")
                    elif [ "${throughput_string:(-4)}" == "kB/s" ] || [ "${throughput_string:(-4)}" == "KB/s" ]
                    then
                        throughput_megabits_per_second=$(bc -l <<<"${throughput_string::(-4)}*1024/1000/1000*8")
                    else
                        # B/s
                        throughput_megabits_per_second=$(bc -l <<<"${throughput_string::(-3)}/1000/1000*8")
                    fi

                    printf "    completion_time_ms             : [${completion_time_ms}] (ms)\n"
                    printf "    requests_per_second            : [${requests_per_second}] \n"
                    printf "    throughput_megabits_per_second : [${throughput_megabits_per_second}] (Mbits/s) \n"
                    ...
                    echo "${throughput_megabits_per_second}" > "${individual_experiment_folder}/throughput_megabits_per_second.txt"
